...
import heapq
import random
from board_nivel_grande import boards_nivel_grande
import subprocess
...
# === Nivel cargado ===
level = boards_nivel_grande
colorTEST = (3, 115, 17)
PI = math.pi

# === Ajuste automático del tamaño de celda ===
ESCALA = 1.2  # Aumenta este valor para más zoom

CELDAS_H = len(level[0])
CELDAS_V = len(level)

CELL_WIDTH = int((Ventana_Ancho // CELDAS_H) * ESCALA)
CELL_HEIGHT = int(((Ventana_Altura - 50) // CELDAS_V) * ESCALA)

# Ajustar dimensiones de ventana al nuevo tamaño total
...
def mostrar_pantalla_inicio():
    frases_buho = [
        "Ey, ya calentaste motores o todavía estás en Bienestar?",
        "Trajiste la tula o vienes sin poderes?",
        "Hoy sí le das con toda... ¿o te devuelves al menú?",
        "Pilas, que esta cabra no se doma con un cafecito.",
        "Esto no es clase, aquí sí te toca correr.",
        "Con fe, como buscando cupo en electiva libre.",
        "Ni el de Física II corre tan rápido como la cabra.",
        "Que no se diga que lo tuyo fue relleno.",
        "Un paso más y te gradúas del Nivel Beta... ¿o no?",
        "No hay simulacro aquí, solo fuego real.",
        "¿Estás más listo que cafetería a las 7 a.m.?",
        "¿Te crees el más teso? La cabra también.",
        "Aquí no hay recuperatorio, solo una vida."
    ]
